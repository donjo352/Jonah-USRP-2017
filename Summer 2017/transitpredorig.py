...
class Star:
    def __init__(self, obj, prio, RA, Dec, P, Epoch, HalfDuration, futype):
        self.obj = obj
        self.prio = prio
        self.RA = RA
        self.Dec = Dec
    ...
    def __repr__(self):
        return repr((self.obj, self.prio, self.RA, self.Dec, self.P,
                     self.Epoch, self.HalfDuration, self.futype))
...
def QueryHATRED(obj):
    ...
    db.cur = db.cursor()
    db.cur.execute('select HTRname, HTRra, HTRdec, HTRP, 2400000+HTRE+0.5*HTRP*HTRq, HTRq*0.5*HTRP, HTRTODO from HTR where HTRname=+"'+obj+'"')
	...
	if int(db.cur.rowcount) > 0:
		...
		if str(row2[3]) == "None":
		    ...
		    b2 = float(row2[3])
                ...
                return ( row[1], row[2], row[3], Tcsec, HalfDursec, prio, futype )
        # just increase the secondary time by 0.5*P
        Tcsec = float(row[4]) + 0.5*float(row[3])
        return ( row[1], row[2], row[3], Tcsec, row[5], prio, futype)
    vals = ( row[1], row[2], row[3], row[4], row[5], prio, futype )
    return vals
...
def QueryALLHATRED():
    ...
    db = MySQLdb.connect(host='hat.astro.princeton.edu',user=dbuser,passwd=dbpwd,db='HATRED')
    db.cur = db.cursor()
    db.cur.execute('select HTRname, HTRra, HTRdec, HTRP, 2400000+HTRE+0.5*HTRP*HTRq, HTRq*0.5*HTRP, HTRTODO from HTR where HTRTODO is not NULL')
    objects = []
                ...
                if int(db.cur.rowcount) > 0:
                    ...
                    if str(row2[1]) != "None" and str(row2[2]) != "None":
                        ...
                        if str(row2[3]) == "None":
                            ...
                            b2 = float(row2[3])
                        [Tcsec, HalfDursec] = SecondaryParams(h, k, b2, float(row[3]), float(row[4]), float(row[5]))
                        objects.append(Star(row[0], prio, float(row[1]), float(row[2]), float(row[3]), Tcsec, HalfDursec, futype))
                        dumcheck=1
            # just increase the secondary time by 0.5*P
                if dumcheck == 0:
                    Tcsec = float(row[4]) + 0.5*float(row[3])
                    objects.append(Star(row[0], prio, float(row[1]), float(row[2]), float(row[3]), Tcsec, float(row[5]), futype))
            else:
                objects.append(Star(row[0], prio, float(row[1]), float(row[2]), float(row[3]), float(row[4]), float(row[5]), futype))
    return objects
...
def QueryHSCAND(obj):
    ...
    db.cur = db.cursor()
    db.cur.execute('select HATSname, HATSra, HATSdec, HATSP, 2400000+HATSE+0.5*HATSP*HATSq, HATSq*0.5*HATSP, HATSTODO from HATS where HATSname=+"'+obj+'"')
    if int(db.cur.rowcount) < 1:
...
def QueryALLHSCAND():
    ...
    db = MySQLdb.connect(host='hatsouth.astro.princeton.edu',user=dbuser,passwd=dbpwd,db='HSCAND')
    db.cur = db.cursor()
    db.cur.execute('select HATSname, HATSra, HATSdec, HATSP, 2400000+HATSE+0.5*HATSP*HATSq, HATSq*0.5*HATSP, HATSTODO from HATS where HATSTODO is not NULL')
    objects = []
    rows = db.cur.fetchall()
    for row in rows:
        prio="..."
