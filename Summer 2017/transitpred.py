...
class TransitEvent:
    ...
    def priority(self):
        ...
        P_period = self.starobj.P/(avgperiod) 
        ...
        minper = 0.114018/(avgperiod)
        maxper = 32.9719/(avgperiod)
...
schedule=0
try:
    opts, args = getopt.getopt(sys.argv[1:], "d:l:p:q:", ["start=", "stop=", "lon=", "lat=", "tz=", "obj=", "name=", "ra=", "dec=", "tc=", "secondary", "allHN", "allHS", "allTEP", "twialt=", "objalt=", "magv=", "schedule"])
...
alltransits = []
for st in Objects:
    alltransits.extend(FindVisibleTransits(yrstart, mostart, daystart, yrstop, mostop, daystop, st.RA, st.Dec, st.P, st.Epoch, st.HalfDuration, float(lat), float(lon), float(tz), float(TwiAlt), float(ObjAlt), st.obj, st.prio, st.futype, st.magv, st))
